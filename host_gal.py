...
if UVOT == 1:
    ...
    ponderation = YOUNG_Sersic(ouv0,Reff, N)
    print("Fraction of the host galaxy luminosity through an aperture of 5 arsec:", ponderation , "\n")
    ...
    for i in range(len(freq_min)):
        ...
        flux_pond_uvot.append(moy(flux)*ponderation)
        flux_moy.append(moy(flux))
    flux_moy_uvot = flux_moy
        
   

    print("Host galaxy flux in UVOT filters without aperture correction [erg.cm-2.s-1]: \n", flux_moy,"\n")
    print("Host galaxy flux in UVOT filters within 5 arcsec aperture [erg.cm-2.s-1]: \n",flux_pond_uvot ,"\n")
    print("UVOT flux removed from the host galaxy emission \n", array(F_uvot)- array(flux_pond_uvot), "\n")
    print("Percentage of host contamination UVOT \n", array(flux_pond_uvot) /(array(F_uvot)- array(flux_pond_uvot))*100)
    
    print("Host B-V UVOT=",-2.5*log10(flux_moy[1]/flux_moy[0]))
